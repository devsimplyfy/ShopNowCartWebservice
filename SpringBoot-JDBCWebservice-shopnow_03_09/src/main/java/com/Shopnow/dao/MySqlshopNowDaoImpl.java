...
public class MySqlshopNowDaoImpl implements shopNowDao {
	...
	public String getCartItemByAdd(CartItemByAdd a1) {
		...
		final int userid = a1.getUserId();
		
		final int product_id = a1.getProduct_id();
		int userid1 = 0;
		int product_id1 = 0;
		int quntity = a1.getQuntity();
		...
		if(quntity<=0) {
			...
			Str_msg = "Cart_Item not Inserted beacuse quntity is not valid";
		...
		while (srs_product_id.next()) {
			...
			product_id1 = srs_product_id.getInt("id");

		}

		while (srs_customer_id.next()) {

			userid1 = srs_customer_id.getInt("id");

		}

		if (product_id1 > 0 && userid1 > 0 && quntity > 0) {

			String sql = "INSERT  INTO cartitems (UserId,ProductId,VendorId,Quantity) VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE  Quantity =(Quantity+"
					+ quntity + ")";

			int i = jdbcTemplate.update(sql, new Object[] { userid, product_id, vendor_id, quntity });
		...
		} else if (product_id1 == 0 && userid1 > 0) {
			Str_msg = "Cart_Item not Inserted Either Product is not available or Product is out of stock";
			jo_cartInsert.put("status", "0");
			jo_cartInsert.put("message", Str_msg);

		} else if (product_id1 > 0 && userid1 == 0) {
			Str_msg = "Cart_Item not Inserted beacuse UserId is not valid";
			jo_cartInsert.put("status", "0");
			jo_cartInsert.put("message", Str_msg);

		} else if (product_id1 == 0 && userid1 == 0) {

			Str_msg = "Cart_Item not Inserted beacuse UserId  and productId are not valid";
			jo_cartInsert.put("status", "0");
			jo_cartInsert.put("message", Str_msg);
		} else {

			Str_msg = "Cart_Item not Inserted beacuse Quntity = 0";
			jo_cartInsert.put("status", "0");
			jo_cartInsert.put("message", Str_msg);
	...
	public String getCartItemByDelete(CartItemByDelete a1) {
		...
		if (srs_customer_id.next() == false) {
			...
			Str_msg = "Cart_Item not Deleted Enter Valid User Id";
			jo_cartInsert.put("status", "0");
			jo_cartInsert.put("message", Str_msg);
			return jo_cartInsert.toString();
		}
     ...
     if(quntity<=0) {
			...
			Str_msg = "Cart_Item not deleted beacuse quntity is not valid";
			jo_cartInsert.put("status", "0");
			jo_cartInsert.put("message", Str_msg);
			return jo_cartInsert.toString();
		}
		...
		SqlRowSet srs_product_id = jdbcTemplate.queryForRowSet(
				"SELECT * FROM cartItems where ProductId='" + product_id + "' AND UserId='" + userid + "'");

		if (srs_product_id.next()) {

			j = srs_product_id.getInt("Quantity");

			if (j > quntity) {

				quntity = j - quntity;

				String sql = "UPDATE Cartitems SET Quantity=" + quntity
						+ " , modified_date = CURRENT_TIMESTAMP() where ProductId='" + product_id + "'AND UserId='"
						+ userid + "'";

				int i = jdbcTemplate.update(sql, new Object[] {});

				Str_msg = "successfuly Update Product Quntity from Cart";
				jo_cartInsert.put("status", "0");
				jo_cartInsert.put("message", Str_msg);

				return jo_cartInsert.toString();

			} else if (j == quntity || srs_product_id.getInt("Quantity") == 0)

			{

				String sql = "DELETE From cartitems  where ProductId='" + product_id + "'AND UserId='" + userid + "'";

				int i = jdbcTemplate.update(sql, new Object[] {});

				Str_msg = "DELETE ROW From cartitems";
				jo_cartInsert.put("status", "0");
				jo_cartInsert.put("message", Str_msg);
				...
				return "You Remove Product Quntity is Greter than Cart Product Quntity ";
			}
		}
		return "Products not Remove from Cart Beacose Product is not presant in Cart ";
	...
	public String getAllCartItem() {
		...
		String sql = "SELECT t1.total,cartItems.* FROM (SELECT CartId,SUM(Price * Quantity) AS total FROM cartItems) AS t1 INNER JOIN cartItems ON t1.CartId=cartItems.CartId LIMIT 10";
	...
	private static class shopNowevent_CartItemByCustomer_IdRowMapper implements RowMapper<JSONObject> {
		...
		public JSONObject mapRow(ResultSet rs, int i) throws SQLException {
			...
			do {
				...
				if (rs.getString("delavery") != null) {
					City = rs.getString("delavery");
				} else {
					City = "Dakor";
				}
			...
			jo_CartItem_Result.put("Delivery To", City);
	...
	public String getCartItemByCustomer_Id(CartItemByCustomer_Id a1) {
		...
		if (srs_customer_id.next() == false) {
			...
			return "Enter The Valid user id";
		}
		...
		String sql = "SELECT Products.*,table1.attribute_value,cartItems.UserId,cartItems.Quantity,SUM(cartItems.Quantity *Products.sale_price) AS total,product_offers.offer_name ,(SELECT GROUP_CONCAT(firstName,\" \",lastName,\", phoneNumber- \",phoneNumber ,\" , Address - \",address1,\" \",address2,\" \",address3,\" \",city,\" \",state) AS addresscustomer  FROM(SELECT * FROM  address WHERE customerId="
				...
				+ " LEFT JOIN cartItems ON cartItems.ProductId=Products.id  LEFT JOIN product_offers ON product_offers.product_id=Products.id WHERE cartItems.UserId="
	...
	public String getPlaceOrder(PlaceOrder a1) {
		...
		final int userid = a1.getUserId();
		String Str_msg;
		JSONObject jo_PlaceOrder = new JSONObject();
		...
		if (srs_customer_id.next() == false) {
			...
			Str_msg = "Enter The Valid user id";
			jo_PlaceOrder.put("status", "0");
			jo_PlaceOrder.put("message", Str_msg);
			return jo_PlaceOrder.toString();
		}

		String paymentMode = a1.getPayment_mode();

		if (paymentMode.isEmpty()) {
            
			Str_msg = "enter valid payment mode not empty! Thank You Try again";
			jo_PlaceOrder.put("status", "0");
			jo_PlaceOrder.put("message", Str_msg);
			return jo_PlaceOrder.toString();
		...
		if ((paymentMode.equalsIgnoreCase("COD") == true)
				|| (paymentMode.equalsIgnoreCase("internet Banking") == true)) {

		} else {
           
			Str_msg = "enter valid payment mode! Thank You Try again";
			jo_PlaceOrder.put("status", "0");
			jo_PlaceOrder.put("message", Str_msg);
			return jo_PlaceOrder.toString();
		...
		orderId = Long.toString(current) + "_" + Integer.toString(userid);

		String sql = "SELECT Products.*,table1.attribute_value,cartItems.UserId,cartItems.Quantity,SUM(cartItems.Quantity *Products.sale_price) AS total ,(SELECT GROUP_CONCAT(firstName,\" \",lastName,\", phoneNumber- \",phoneNumber ,\" , Address\",address1,\" \",address2,\" \",address3,\" \",city,\" \",state) AS addresscustomer  FROM(SELECT * FROM  address WHERE customerId='110' LIMIT 1)AS table1 )AS delavery  FROM Products LEFT JOIN \n"
				+ "(SELECT pa.product_id,GROUP_CONCAT(att_group_name,'\":\"',av.att_value) AS attribute_value FROM product_attributes pa INNER JOIN attributes_value av ON av.id=pa.att_group_val_id INNER JOIN attributes a ON a.id=pa.att_group_id GROUP BY pa.product_id) AS table1 ON Products.id=table1.product_id \n"
				+ " LEFT JOIN cartItems ON cartItems.ProductId=Products.id  WHERE  cartItems.UserId=" + userid
		...
		while (srs_product_id.next()) {
			...
			productId = srs_product_id.getInt("id");
			quntity = srs_product_id.getInt("Quantity");
			...
			if (quntity > 0) {
				...
				if (day == "Sunday") {
					...
					String sql_insert_address = "INSERT INTO order_delivery(order_id,user_id,product_description, productId,payment_status,delivery_status_code,delivery_address,vendorId,vendor_product_id,quantity,mode_of_payment,transaction_id,discounts,delivery_charges,price,expected_date_of_delivery) VALUES\n"
							+ "(?,?,?,?,'COD','200 ok',?,?,?,?,?,'1245648',100,200,?,DATE_ADD(Now(), INTERVAL 8 DAY))";
					i = jdbcTemplate.update(sql_insert_address, new Object[] { orderId, userid, name, productId,
							delavery, vendorId, vendor_product_id, quntity, paymentMode, sale_price });

				} else {

					String sql_insert_address = "INSERT INTO order_delivery(order_id,user_id,product_description, productId,payment_status,delivery_status_code,delivery_address,vendorId,vendor_product_id,quantity,mode_of_payment,transaction_id,discounts,delivery_charges,price,expected_date_of_delivery) VALUES\n"
							+ "(?,?,?,?,'COD','200 ok',?,?,?,?,?,'1245648',100,200,?,DATE_ADD(Now(), INTERVAL 7 DAY))";
					i = jdbcTemplate.update(sql_insert_address, new Object[] { orderId, userid, name, productId,
							delavery, vendorId, vendor_product_id, quntity, paymentMode, sale_price });
		...
		// -----------------------------payment web  service------------------------------------------------------------------
		long card_number1 = 0L;
		try {
			card_number1 = a1.getCard_number();
		} catch (Exception e) {

			System.out.println("Please enter valid cart number");

		}

		int length_card = String.valueOf(card_number1).length();
		String name_on_card = a1.getName_on_card();
		String cardtype = a1.getCardtype();

		Calendar cal = Calendar.getInstance();
		int Cmonth = cal.get(Calendar.MONTH);
		int expiryMonth = a1.getExpiryMonth();
		int expiryYear = a1.getExpiryYear();

		int Cyear = (Year.now().getValue()) % 100;

		if (length_card != 16) {
			
			Str_msg = "Enter Valid Card Number! Thank You Try again";
			jo_PlaceOrder.put("status", "0");
			jo_PlaceOrder.put("message", Str_msg);
			return jo_PlaceOrder.toString();
		...
		if (expiryMonth > 13 || expiryMonth == 0 || Cyear > expiryYear) {

			Str_msg = "please Check Your Card Expiry! Thank You Try again";
			jo_PlaceOrder.put("status", "0");
			jo_PlaceOrder.put("message", Str_msg);
			return jo_PlaceOrder.toString();
		...
		if ((cardtype.equalsIgnoreCase("CD") == true) || (cardtype.equalsIgnoreCase("DC") == true)) {
		} else {
			
			Str_msg = "Check Your Payment Type! Thank You Try again";
			jo_PlaceOrder.put("status", "0");
			jo_PlaceOrder.put("message", Str_msg);
			return jo_PlaceOrder.toString();
		...
		if (name_on_card != null && expiryMonth > Cmonth) {

			JSONArray json_array_orderItem1 = new JSONArray();
			JSONObject jo_CartItem_Result = new JSONObject();

			String payment_sql = "SELECT card_number,name_on_card,card_number,expiry_month,expiry_year FROM customer_payment_methods WHERE customer_id ='"
					+ userid + "' and  card_number=" + card_number1;
			SqlRowSet payment_detail = jdbcTemplate.queryForRowSet(payment_sql);

			String sql_insert_card_detail = "INSERT IGNORE  INTO customer_payment_methods(customer_id,payment_method_code,name_on_card,card_number,expiry_month,expiry_year) VALUES(?,'"
					+ cardtype + "','" + name_on_card + "',?," + expiryMonth + "," + expiryYear + ")";
			i = jdbcTemplate.update(sql_insert_card_detail, new Object[] { userid, card_number1 });
			System.out.println(i);

			if (i > 0) {

				jo_CartItem_Result.put("Card_Detail", "insert into payment table");
			} else {

				while (payment_detail.next()) {

					JSONObject jo_CartItem_Result1 = new JSONObject();
					jo_CartItem_Result1.put("Name On Card", payment_detail.getString("name_on_card"));
					jo_CartItem_Result1.put("Card Number", payment_detail.getString("card_number"));
					int m = payment_detail.getInt("expiry_month");
					String m1 = Integer.toString(m);
					String s = payment_detail.getDate("expiry_year").toString().substring(0, 4);
					jo_CartItem_Result1.put("expiry", m1 + "/" + s);
					json_array_orderItem1.put(jo_CartItem_Result1);

				}
				jo_CartItem_Result.put("Card_Detail", json_array_orderItem1);

			}
			Str_msg = " Thank you! Your Order - " + orderId + " Placed successfully \n" + jo_CartItem_Result.toString();

		} else {

			Str_msg = "Enter Valid Expiry Month";

		}
		// SqlRowSet srs_product_id1 = jdbcTemplate.queryForRowSet("Delete from
		// cartitems WHERE Userid="+userid);
	...
	public String getProductOrder(ProductOrder a1) {
		...
		int userId = a1.getUserId();
		String Str_msg;
		JSONObject jo_OrderDisplay = new JSONObject();
		
		
		SqlRowSet srs_customer_id1 = jdbcTemplate.queryForRowSet("SELECT id FROM customers where id='" + userId + "'");

		if (srs_customer_id1.next() == false) {

			Str_msg = "Enter The Valid user id";
			jo_OrderDisplay.put("status", "0");
			jo_OrderDisplay.put("message", Str_msg);
			return jo_OrderDisplay.toString();
		}

		
		String orderBy = a1.getOrderBy();
		String search = a1.getSearch();
		int pagenumber = a1.getPageNumber();
		String order1;
		...
		int page_offset = (pagenumber - 1) * 10;
		...
		while (srs_customer_id.next()) {
			...
			product_id1 = srs_customer_id.getInt("id");

		}

		if (product_id1 > 0) {

			String sql1 = "select order_id,delivery_status_code,vendorId,mode_of_payment,transaction_id,delivery_address,order_Date_Time,expected_date_of_delivery,payment_status from order_delivery where user_id ='"
					+ userId + "' and product_description LIKE '%" + search + "%' GROUP BY order_id ORDER BY id "
					+ order1 + " Limit " + page_offset + ", 10";

			SqlRowSet str_order_ids = jdbcTemplate.queryForRowSet(sql1);
			...
			while (str_order_ids.next()) {
				...
				String sql = "SELECT * FROM order_delivery  INNER JOIN products ON products.id=order_delivery.productId WHERE order_delivery.order_id ='"
				...
				while (str_order_product.next()) {
					...
					jo_OrderItem1.put("product_id", str_order_product.getInt("productId"));
					jo_OrderItem1.put("product_name", str_order_product.getString("product_description"));
					jo_OrderItem1.put("vendor_id", str_order_ids.getString("vendorId"));
				...
				jo_CartItem_Result.put("delivery_status_code", str_order_ids.getString("delivery_status_code"));
				...
				if (str_order_ids.getString("mode_of_payment") != "COD") {
					...
					jo_CartItem_Result.put("payment_status", str_order_ids.getString("payment_status"));
				...
				jo_CartItem_Result.put("billing address", str_order_ids.getString("delivery_address"));
				jo_CartItem_Result.put("shipping_address", str_order_ids.getString("delivery_address"));
				String order = "order" + i;
				jo_CartItem_Result.put(order, json_array_orderItem1);
				i++;
